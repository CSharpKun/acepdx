...
{
    public static IEnumerable<T> GetRelevantLicenses<T>(
        this IEnumerable<T> licenses,
        string licenseId,
        ...
        where T : ILicense
    {
        return licenses
            .Where(license =>
                license.LicenseId.Contains(licenseId, StringComparison.InvariantCultureIgnoreCase)
                || license.LicenseId.StartsWith(
                    licenseId,
            ...
            .ThenBy(license =>
            {
                if (license.LicenseId.EndsWith("-only"))
                    return 0;
                if (license.LicenseId.EndsWith("-or-later"))
                    return 1;
                return 2;
            })
